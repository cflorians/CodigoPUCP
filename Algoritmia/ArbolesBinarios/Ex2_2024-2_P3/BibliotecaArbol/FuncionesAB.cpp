...
void insertarRecursivo(Nodo*& raiz, const Elemento& elemento) {
    ...
    if (raiz->elemento.id > elemento.id) { // si es menor nos vamos a la izquierda
        insertarRecursivo(raiz->izq, elemento);
    }
    else { // si es mayor igual nos vamos a la derecha
...
Nodo* eliminar(Nodo*& raiz, int valor) {
    ...
    if (valor < raiz->elemento.id) {
    ...
    else if (valor > raiz->elemento.id) {
        // buscamos a su der
        ...
        raiz->der = eliminar(raiz->der, elementoSucesor.id);
    }
    return raiz;
}
...
void imprimeNodo(const Nodo *nodo) {
    cout << setw(10) << nodo->elemento.flag << "-" << nodo->elemento.id << "  ";
}
...
Nodo* buscaElemento(const ArbolBinario& arbol, const Elemento& elemento) {
    return buscaElementoRecursivo(arbol.raiz, elemento);
}

Nodo* buscaElementoRecursivo(Nodo* nodo, const Elemento& elemento) {
    if (esNodoVacio(nodo)) return nullptr;

    if (elemento.id == nodo->elemento.id) return nodo;
    else if (elemento.id > nodo->elemento.id) return buscaElementoRecursivo(nodo->der, elemento);
    else return buscaElementoRecursivo(nodo->izq, elemento);
}