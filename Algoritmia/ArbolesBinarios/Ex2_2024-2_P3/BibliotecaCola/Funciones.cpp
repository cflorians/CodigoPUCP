...
void ordenarCola(struct Cola& cola, int n) {
    ...
    for (int i = 1; i <= n; i++) { // total de iteraciones = 10
        ...
        for (int j = 1; j <= n-i; j++) { // total de iteraciones = 9
            aux = desencolar(cola);
            if (aux.id < menor.id) { // Si encuentro un nuevo menor
                encolar(cola, menor);
...
void invertirCola(Cola& cola) {
    ...
    while (not esColaVacia(cola)) {
        ...
        apilar(pilaAux, {elem.flag, elem.id});
    }

    while (cima(pilaAux)) {
        ElementoPila elem_pila = desapilar(pilaAux)->elemento;
        encolar(cola, {elem_pila.flag, elem_pila.id});
    }
}