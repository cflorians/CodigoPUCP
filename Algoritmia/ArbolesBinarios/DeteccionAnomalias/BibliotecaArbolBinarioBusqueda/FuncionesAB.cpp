...
Nodo* encuentraMaximo(Nodo* nodo) {
    while (nodo->der) {
        nodo = nodo->der;
    }
    return nodo;
...
bool esABB(Nodo* nodo) { // REHACER CON MAXIMO Y MINIMO
    // para la verificacion, hacemos nivel a nivel y además verificamos que no este
    // duplicado en otra parte del arbol
    if (nodo == nullptr) return true;

    int maxIzq = maxABB(nodo->izq);
    int minDer = minABB(nodo->der);

    if (maxIzq >= nodo->elemento.numero and minDer <= nodo->elemento.numero) {
        return false;
    }
    return esABB(nodo->izq) and esABB(nodo->der);
}

// FUNCIONES TRAIDAS DEL ARBOL BINARIO

int minABB(Nodo* raiz) {
    if (raiz == nullptr) return INT_MAX;

    Elemento res = raiz->elemento;
    int minIzq = minABB(raiz->izq);
    int minDer = minABB(raiz->der);

    return min(res.numero, minIzq, minDer);
}

int maxABB(Nodo* raiz) {
    if (raiz == nullptr) return INT_MIN;

    Elemento res = raiz->elemento;
    int maxIzq = maxABB(raiz->izq);
    int maxDer = maxABB(raiz->der);

    return max(res.numero, maxIzq, maxDer);
}

int max(int a, int b, int c) {
    if (a>b and a>c) return a;
    if (b>a and b>c) return c;
